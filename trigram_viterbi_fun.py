...
def trigram_viterbi(hmmfile, textfile):
    ...
    # Read in the HMM and store the probabilities as log probabilities
        for line in f:
            if line.startswith('trans'):
                ...
                tri_state = tuple(line_split[1:4])
                states[tri_state] = 1
    ...
    output_file_path = "vocab_tri.txt"
...
test = trigram_viterbi('my_trigram.hmm', 'data/ptb.22.txt')
#test = trigram_viterbi('btb_tri.hmm', 'data/btb.test.txt')
#test = trigram_viterbi('jv_tri.hmm', 'data/jv.test.txt')